 ...
 * [] a way to derive the next state using only the
...
for (let i = 1; i <= data.length - sublist_size; i++) {
  window_sum = window_sum - data[i - 1] + data[i + sublist_size - 1];
  sliding_window_averages.push(Math.round(window_sum / 3));
}

console.log(sliding_window_averages);
